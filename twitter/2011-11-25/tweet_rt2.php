...
// in_reply_to_status_idを指定するのならば array("status"=>"@hogehoge reply","in_reply_to_status_id"=>"0000000000"); とする。
	...
	$tweet_text = $gyoukai[$sheet_num]."業界年収".$inc_arr[$inc_num][0]."位、".$inc_arr[$inc_num][1]."の"."平均年収は ".$inc_arr[$inc_num][2]."万円。 #年収";
	fclose($fp); 
	if(empty($inc_arr[$inc_num][0])){
		if($sheet_num % 2 == 1){
		$tweet_text = "こちらは年収botです。各社の年収についてつぶやきます。 #年収";
		}else{
